...
def summary():
    ...
    cur.execute("SELECT COUNT(*) AS c FROM posts WHERE location='ideas' AND status='idea'")
    fresh_ideas = cur.fetchone()['c']
    cur.execute("SELECT title, planned_date FROM posts WHERE location='planner' AND planned_date IS NOT NULL AND DATE(planned_date)=DATE('now') ORDER BY planned_date LIMIT 1")
    due_today = cur.fetchone()
    cur.execute("SELECT title, planned_date FROM posts WHERE location='planner' AND planned_date IS NOT NULL AND DATE(planned_date) > DATE('now') AND DATE(planned_date) <= DATE('now', '+3 day') ORDER BY planned_date LIMIT 1")
    next_due = cur.fetchone()
    conn.close()

    todos = []
    if due_today:
        todos.append({'text': f'Heute fällig: {due_today["title"]}', 'detail': f'Geplant für {due_today["planned_date"]}', 'kind': 'planned', 'priority': 'high'})
    if work_items:
        todos.append({'text': f'{work_items} Beitrag/Beiträge sind noch in Arbeit', 'detail': 'Diese Posts brauchen noch Feinschliff, Copy oder Export.', 'kind': 'work', 'priority': 'high' if work_items > 2 else 'medium'})
    if missing_dates:
        todos.append({'text': f'{missing_dates} geplanter Beitrag/Beiträge haben noch kein Datum', 'detail': 'Ohne Datum ist der Kalender eher eine schöne Idee.', 'kind': 'idea', 'priority': 'high'})
    if next_due:
        todos.append({'text': f'In den nächsten 3 Tagen fällig: {next_due["title"]}', 'detail': f'Geplant für {next_due["planned_date"]}', 'kind': 'planned', 'priority': 'medium'})
    if missing_notes:
        todos.append({'text': f'{missing_notes} geplanter Beitrag/Beiträge haben noch keine Notizen', 'detail': 'Kann später beim Produzieren unnötig weh tun.', 'kind': 'idea', 'priority': 'low'})
    if fresh_ideas:
        todos.append({'text': f'{fresh_ideas} Idee/Ideen warten noch auf Einplanung', 'detail': 'Vielleicht steckt dein nächster starker Post schon dort drin.', 'kind': 'idea', 'priority': 'low'})
